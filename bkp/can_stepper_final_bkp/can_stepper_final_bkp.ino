...
//this set's a new value to thbe home 
void calibrateRotor() {
  Serial.println ("\nCalibrating Rotor");
...
//function for a quick re-calibaration at 0 degree
...
void loop() {
  ...
  //if there is a new message event
  if (can.available()) {
    ...
    if (frame.data[0]==1) calibrateRotor();

    //if already calibrated and it's not a calibration request, 
    ...
    if(angle==0 && frame.data[0]==0)  microCalib=0;
  }
  ...
  //micro calibrating, after waiting for rotor to finish reaching 0 degree
  if (microCalib==0 && inMotion == 0 && angle == 0) microCalibrateRotor();
